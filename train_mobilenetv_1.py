...
from my_dataset_1 import KITTIDataSet
...
def create_model(num_classes):
    ...
    backbone = MobileNetV2(weights_path="./src/faster_rcnn/backbone/mobilenet_v2-b0353104.pth").features
...
def main():
    ...
    kitti_root = "./"  # VOCdevkit
    aspect_ratio_group_factor = 3
    batch_size = 2
    amp = False  # 是否使用混合精度训练，需要GPU支持

    # check voc root
    if os.path.exists(os.path.join(kitti_root,"training")) is False:
        raise FileNotFoundError("VOCdevkit dose not in path:'{}'.".format(kitti_root))

    # load train data set
    # VOCdevkit -> VOC2012 -> ImageSets -> Main -> train.txt
    train_dataset = KITTIDataSet(kitti_root, data_transform["train"], "train.txt")
    train_sampler = None
    ...
    val_dataset = KITTIDataSet(kitti_root, data_transform["val"], "val.txt")
